...
Wrap.prototype.readFile = function (file) {
    ...
    self.filters.forEach(function (fn) {
        var r = fn.call(self, body, file, map);
...
Wrap.prototype.addEntry = function (file_, opts) {
    ...
    var body = entry.body = opts.body || self.readFile(file).body;
...
Wrap.prototype.bundleSourceMap = function() {
    var lines = function(string) { return string.match(/\n/).length };

    var map = {version: 3, file: 'test.js', sections: []};
    var nextLine = lines(this.prelude);

    Object.forEach(function (file, s) {
        var target = s.target || removeLeading(file);
        var wrapOffset = self.wrapOffset(target);
        nextLine += wrapOffset.before.lines;
        map.sections.push({
          offset: {line: nextLine, column: wrapOffset.column},
          map: s.map
        });
        nextLine += s.body + wrapOffset.after.lines + 1;
        return section;
    });
}
...
Wrap.prototype.wrapOffset = function(target) {
    var lines = function(string) { return string.match(/\n/).length };
    var column = function(string) { return string.match(/\n[^\n]*$/)[0].length - 1 }

    var self = this;
    var wrapper = self.debug ? wrappers.body_debug : wrappers.body;
    
    var wrapped = wrapper.replace(/\$__filename/g, function() {return JSON.stringify(target);});
    var index = wrapped.indexOf("$body");

    var wrappedBefore = wrapped.slice(0, index);
    var wrappedAfter = wrapped.slice(index + "$body".length);

    return {
      before: {lines: lines(wrappedBefore), column: column(wrappedBefore)},
      after: {lines: lines(wrappedAfter)}
    }
}
...
Wrap.prototype.require = function (mfile, opts) {
    ...
    // entry must be set before readFile for the watch
    var body = entry.body = opts.body || self.readFile(opts.file).body;
    if (body === undefined) {
